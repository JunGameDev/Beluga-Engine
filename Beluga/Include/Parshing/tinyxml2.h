...
private:
    ...
    void EnsureCapacity( int cap ) {
        ...
        if ( cap > _allocated ) {
            ...
            Type* newMem = new Type[newAllocated];
            TIXMLASSERT( newAllocated >= _size );
            memcpy( newMem, _mem, sizeof(Type)*_size );	// warning: not using constructors, only works for PODs
            if ( _mem != _pool ) {
                delete [] _mem;
            }
            _mem = newMem;
